...
final public class TieHandler implements EndpointCallBridge {
    ...
    private EndpointArgumentsBuilder createArgumentsBuilder() {
        ...
        // CODE FOR JSONCODEC
        if(rp.size() == 1 && rp.get(0).isWrapperStyle() && this.binding.getBindingID().equals(JSONBindingID.JSON_BINDING)){
        	List<ParameterImpl> children = ((WrapperParameter)rp.get(0)).getWrapperChildren();
        	if(children.size()>1){
        		rp=children;
        	}
        }
        ...
        for( ParameterImpl param : rp ) {
            ...
            case BODY:
                if(param.isWrapperStyle()) {
                	// CODE FOR JSONCODEC
                	if(this.binding.getBindingID().equals(JSONBindingID.JSON_BINDING)){
